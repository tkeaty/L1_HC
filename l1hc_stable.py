...
def hill_climb2(data, pcs):
    ...
    while not done:
        ...
        new_p, new_c, mode, ll, prune_cands = select_edge2(data, g, candidate_edges)
        if new_p == -1 or ll < 0:
            done = True
            break
        ...
        g = preprocess_single_prob(g, data, new_p, new_c, mode)
...
def mmhc(data, selection_method='l1'):
    ...
    # Do hill climbing using heuristic scoring
...
# def pairwise_dispersion(data1, data2, n_keep=100):
#     mean1 = np.mean(data1.values, axis=0)
#     mean2 = np.mean(data2.values, axis=0)
#
#     std1 = np.var(data1.values, axis=0)
#     std2 = np.var(data2.values, axis=0)
#
#     disp1 = std1/mean1
#     disp2 = std2/mean2
#
#     inds1 = np.argsort(disp1)
#     inds2 = np.argsort(disp2)
#
#     scores = np.zeros(len(inds1))
#
#     for i in range(len(inds1)):
#         scores[inds1[i]] += len(inds1) - i
#         scores[inds2[i]] += len(inds1) - i
#
#     inds = np.argsort(scores)[-n_keep:]
#
...
if __name__ == '__main__':
    aml_df_raw = pd.read_csv('AML_files/AML_case_expression_TPM.csv')
    aml_df_raw = aml_df_raw.drop(columns='Unnamed: 0')

    ctrl_df_raw = pd.read_csv('AML_files/AML_ctrl_expression_TPM.csv')
    ctrl_df_raw = ctrl_df_raw.drop(columns='Unnamed: 0')

    all_df_raw = pd.read_csv('ALL_files/ALL_case_expression_TPM.csv')
    all_df_raw = all_df_raw.drop(columns='Unnamed: 0')

    print(all_df_raw.shape[0])
    print(aml_df_raw.shape[0])
    print(ctrl_df_raw.shape[0])
